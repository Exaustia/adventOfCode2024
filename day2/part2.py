...
def remove_pair_duplicata(inputs):
    ...
    for line in files:
        ...
        if ordered and least_or_most:
            ...
            for i in range(len(fire_levels)):
                removeCurrentIndex = fire_levels.copy()
                removeCurrentIndex.pop(i)
                ordered = correct_order(removeCurrentIndex)
                least_or_most = at_least_or_at_most(removeCurrentIndex)
                if ordered and least_or_most:
                    SAFE_LINES += 1
                    break
                continue
